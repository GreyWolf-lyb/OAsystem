...
<script>
import { roleInsert, roleUpdate, roleFindById} from '@/api/role'
export default {
  components: {
    // 注册组件
    // editor: editor
  ...
  methods: {
    ...
    async initFormDetail (value) {
      ...
      try {
        ...
        that.loading = true
        const response = await roleFindById(parameter)
        that.loading = false
    ...
    async add(data) {
      ...
      that.loading = true
      const response = await roleInsert(da)
      that.loading = false
